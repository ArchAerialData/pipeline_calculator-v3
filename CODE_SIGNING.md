...
## What’s signed
- The workflow signs the built `.app` with a **Developer ID Application** certificate, then packages it into `dist/Pipeline_Calculator_v5.N_arm64.dmg`.
...
```bash
bash scripts/macos/notarize_dmg.sh dist/Pipeline_Calculator_v5.N_arm64.dmg
```
