...
def calculate_overlap_results(
    ...
    for (p1_idx, p2_idx), segments in parallel_groups.items():
        ...
        if current_section and len(current_section) * segment_length >= min_parallel_length:
            continuous_sections.append(current_section)

        for section in continuous_sections:
            try:
                bundled_length = len(section) * segment_length
    ...
    total_bundled = sum(section["segment_count"] * segment_length for section in results["bundled_sections"])
